...
class Google_Api {
	 ...
	 * @return array
	 */
	public function get_queries($profile, $start_date, $end_date, $limit, $offset, $dimension, $search_type = 'web', $filter_groups = [] ) {

		return $this->post(
            'https://www.googleapis.com/webmasters/v3/sites/'. $profile .'/searchAnalytics/query',
            [
                ...
                'dataState' => 'all',// todo: make dynamic
            ]
        );
	}
	...
	public function revoke_token( $token ) {
		...
		$this->post(
			Request::add_query_arg_raw( [ 'token' => $token ], 'https://oauth2.googleapis.com/revoke' )
		);
	...
	public function get( $url, $args = [], $timeout = 100 ) {
		return $this->do_request( 'GET', $url, $args, $timeout );
	}
	 ...
	 */
	private function do_request( $http_verb, $url, $args = [], $timeout = 10 ) {
		$params = [
			'timeout' => $timeout,
			'method'  => $http_verb,
		];
	...
	private function get_config() {
		$config = [
            'application_name' => 'Sero',
            'redirect_uri'     => 'urn:ietf:wg:oauth:2.0:oob',
			'client_id'        => '1079328382042-2lfcdnss23q0i9imp5crb19cgib36qtf.apps.googleusercontent.com',
            'client_secret'    => '11JIVw-phzq1EXEqe_ptbtgN',
